package dev.rosalyn.neopbb.event.packet
...
class PacketInteraction(val commando: SpigotCommando) : InteractionType<Nothing?, Nothing?>(), PacketListener {
    ...
    override fun createRootNode(parent: KClass<*>): Node<Nothing?> {
        return Node(commando, null, parent.java.name, null)
    }

    override fun parse(root: Node<Nothing?>, parent: KClass<*>, handle: FunctionHandle) {
        root.children += Node(commando, root, handle.name, handle.reflector)
    }
